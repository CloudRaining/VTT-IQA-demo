...
class STNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=1, dim = 128, num_tokens = 8, mlp_dim = 256, heads = 4, depth = 6, emb_dropout = 0.1, dropout= 0.1):
    ...
    def forward(self, img, mask = None, stats='train'):
        ...
        # wv:(b, 64, cT)
        if stats == 'test':
            wa = rearrange(self.token_wA.expand([BATCH_SIZE_TEST,self.L,64]), 'b h w -> b w h')
        else:
            wa = rearrange(self.token_wA.expand([Batch_Size,self.L,64]), 'b h w -> b w h') #Transpose
        ...
        # VV:(b, hw, cT)
        if stats == 'test':
            VV= torch.einsum('bij,bjk->bik', x, self.token_wV.expand(BATCH_SIZE_TEST, 64, self.cT))
        else :
            VV= torch.einsum('bij,bjk->bik', x, self.token_wV.expand(Batch_Size, 64, self.cT))
        # T:(b, L, cT = 128)
